...
function getFieldId(record: FieldRecordAny): string {
  return (
    (record as FieldRecordV4).field_id ??
    (record as FieldRecordV3).settingKey ??
    "unknown"
  );
}

function getCurrentValue(
  record: FieldRecordAny,
...
async function main(): Promise<void> {
  ...
  const threshold = thresholdArg >= 0 ? Number(args[thresholdArg + 1]) : 0.2;
  ...
  const byType = new Map<
    string,
    { total: number; currentNull: number; defaultNull: number }
  >();
  ...
  if (currentNullRate > threshold) {
    process.stdout.write(
      `FAIL: current_value null rate ${pct(totalCurrentNull, totalFields)} exceeds threshold ${(threshold * 100).toFixed(1)}%\n`,
    );
    process.exit(1);
  } else {
    process.stdout.write(
      `PASS: current_value null rate is within threshold.\n`,
    );
  }
}
