...
export type ProfileSaveInput = ProfileIdentity & {
  displayName?: string;
  values: ProfileValueInput[];
};
...
export type ProfileRecord = ProfileIdentity & {
...
export async function saveProfile(dbPath: string, draft: ProfileSaveInput): Promise<ProfileRecord> {
  ...
  try {
    ...
    upsertProfile.run(identity.accountNumber, identity.variation, draft.displayName ?? null, now);
