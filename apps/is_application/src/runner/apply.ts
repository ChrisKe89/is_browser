...
import { buildSettingsFromProfile } from "@is-browser/sqlite-store";
...
async function run(): Promise<void> {
  ...
  if (!accountNumber) {
    throw new Error(
      "Missing APPLY_ACCOUNT_NUMBER. Settings are now loaded from DB profiles only.",
    );
  }
  ...
  const ip =
    ...
    new URL(process.env.PRINTER_URL ?? "http://192.168.0.107").hostname;
  ...
  const settings = {
    ...
    options: {
      ...
      deviceLogMode:
        process.env.APPLY_DEVICE_LOG_MODE === "daily" ||
        process.env.APPLY_DEVICE_LOG_MODE === "all-time"
