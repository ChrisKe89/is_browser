...
function summarize(map: UiMap): Summary {
  ...
  const dropdowns = fields.filter((field) => field.controlType === "dropdown" || field.type === "select");
  const remaining = dropdowns
    .filter((field) => field.currentValue === null || field.currentValue === undefined)
    .map((field) => ({
...
async function resolveMapPath(input: string): Promise<string> {
  ...
  if (await stat(direct).then(() => true).catch(() => false)) {
    return direct;
  }
  const repoRelative = path.resolve(process.cwd(), "..", "..", input);
  if (await stat(repoRelative).then(() => true).catch(() => false)) {
    return repoRelative;
  }
  return direct;
}
...
function printSummary(title: string, summary: Summary): void {
  ...
  console.log(`  dropdowns with null currentValue: ${summary.nullCurrentValue}`);
...
async function main(): Promise<void> {
  ...
  if (args.length === 0 || args.length > 2) {
    throw new Error("Usage: tsx src/verifyDropdownValues.ts <after-map.json> [before-map.json]");
  }
