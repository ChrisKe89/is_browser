...
import {
  ...
  mergeEnums,
} from "./mapping/fieldDiscovery.js";
import { fieldFingerprint } from "./mapping/fingerprint.js";
import {
  buildVisibleFieldDescriptors,
  captureFieldStateSnapshot,
} from "./mapping/fieldStateSnapshot.js";
...
const MODAL_ROOT_SELECTOR =
  "#deviceDetailsModalRoot, .ui-dialog-content, .xux-modalWindow-content, [role='dialog']";
const MODAL_OPEN_LABEL_RE =
  /details|advanced|settings|summary|network|administrator|edit/i;
const MODAL_CLOSE_LABEL_RE = /^(cancel|close|done|ok)$/i;
const SYSTEM_ALERT_TOKEN_RE =
  /securityalert|security alert|certificate|unsafe|warning/i;
...
function isSystemAlertClick(click: CapturedClick): boolean {
  ...
  const selectorProbe = click.selectors
    .map((selector) =>
      normalizeSpace(
        selector.value ?? selector.name ?? selector.role,
      ).toLowerCase(),
    )
    .join(" ");
  ...
  if (
    label &&
    SYSTEM_ALERT_TOKEN_RE.test(label) &&
    /(ok|close|dismiss|continue|back)/i.test(label)
  )
    return true;
  return false;
}
...
function classifyClickKind(click: CapturedClick): CapturedClickKind {
  ...
  if (click.kind && click.kind !== "unknown") {
    if (click.kind === "button" && MODAL_CLOSE_LABEL_RE.test(label))
      return "modal_close";
    return click.kind;
  }
  ...
  if (
    role === "radio" ||
    css.includes("input[type='radio']") ||
    css.includes('input[type="radio"]')
  ) {
    return "radio_select";
  }
  if (role === "combobox") return "combobox";
  if (css.startsWith("select") || css.includes(" select"))
    return "dropdown_trigger";
  if (MODAL_CLOSE_LABEL_RE.test(label)) return "modal_close";
  if (
    MODAL_OPEN_LABEL_RE.test(label) &&
    (role === "button" || role === "link" || css.includes("button"))
  ) {
    return "modal_open";
  }
...
function sameArea(first: CapturedClick, second: CapturedClick): boolean {
  if (first.frameUrl && second.frameUrl && first.frameUrl !== second.frameUrl)
    return false;
...
function shouldCollapseWrapper(
  first: CapturedClick,
  second: CapturedClick,
): boolean {
...
function shouldCollapseRadioSequence(
  first: CapturedClick,
  second: CapturedClick,
): boolean {
  const secondKind = classifyClickKind(second);
  ...
  if (!secondLabel.includes(firstLabel) && !firstLabel.includes(secondLabel))
    return false;
  const firstMs = toMillis(first.timestamp);
  const secondMs = toMillis(second.timestamp);
  if (
    Number.isFinite(firstMs) &&
    Number.isFinite(secondMs) &&
    secondMs - firstMs > 1200
  )
    return false;
  return true;
}

function mergedRadioClick(
  first: CapturedClick,
  second: CapturedClick,
): CapturedClick {
...
function inferTransitionType(
  ...
  if (kind === "system_alert" || kind === "dismiss_alert")
    return "dismiss_alert";
  if (beforeScope.kind !== "modal" && afterScope.kind === "modal")
    return "open_modal";
  if (beforeScope.kind === "modal" && afterScope.kind !== "modal")
    return "close_modal";
  ...
  if (urlBefore && normalizedAfter && urlBefore !== normalizedAfter)
    return "navigate";
  if (kind === "link") return "navigate";
  return "expand_section";
}

function shouldRevealFromTransition(
  ...
  return ["open_modal", "navigate", "tab_switch", "expand_section"].includes(
    transitionType,
  );
}
...
function mergeFieldIntoExisting(
  existing: FieldEntry,
  incoming: FieldEntry,
): void {
...
async function resolveScopeContext(
  ...
  const modalId = normalizeSpace(
    (await modalRoot.getAttribute("id").catch(() => null)) ?? undefined,
  );
  const modalClass = normalizeSpace(
    (await modalRoot.getAttribute("class").catch(() => null)) ?? undefined,
  )
    .split(/\s+/)
    .slice(0, 3)
    .join(".");
  const frameUrl = normalizeSpace(page.url());
  const modalIdentity = stableModalId(
    frameUrl,
    modalTitle,
    modalId,
    modalClass,
...
async function derivePageContext(
  ...
  const pageId =
    ...
    uniqueId(
      slugify(
        `${title ?? "page"}-${activeScope.kind === "modal" ? "modal" : pathname}`,
      ) || "page",
      pageIds,
    );
  if (!existingPageId) {
...
function clickSelectorToNavSelector(
  click: CapturedClick,
): Selector | undefined {
...
async function waitForClick(
  clickQueue: ClickCaptureQueue,
  isStopRequested: () => boolean,
): Promise<CapturedClick | undefined> {
...
async function normalizeClickSequence(
  clickQueue: ClickCaptureQueue,
  first: CapturedClick,
): Promise<CapturedClick | undefined> {
  await sleep(120);
...
export async function runManualMapper(
  options: MapperCliOptions,
): Promise<void> {
  requireCreds();
  ...
  const resolvedRunPaths = resolveManualRunPaths({
    location: options.location,
  });
  const runPaths = await ensureManualRunPaths(
    resolvedRunPaths,
    options.screenshot,
  );
  ...
  const initialPage = await derivePageContext(
    page,
    registry.pageIds,
    registry.pageContextIds,
    navPath,
    initialScope,
  );
  pagesById.set(initialPage.id, initialPage);
  const initial = await collectNewFields(
    page,
    initialPage,
    registry,
    runId,
  ...
  visibleBaselineByScope.set(
    initial.scopeKey,
    new Set(initial.visibleFieldIds),
  );
  ...
  const initialSnapshot = await captureFieldStateSnapshot(
    page,
    initialSnapshotDescriptors,
    initialScope.root,
  );
  ...
  console.log(
    "Manual mapper is running. Click through the UI. Enter 'q' then Enter to finish.",
  );

  while (!stopRequested) {
    if (
      typeof options.maxClicks === "number" &&
      clickLogEntries.length >= options.maxClicks
    ) {
    ...
    const pageEntry = await derivePageContext(
      page,
      registry.pageIds,
      registry.pageContextIds,
      navPath,
    ...
    const {
      newFields,
      newFieldIds: newlyDiscoveredFieldIds,
      visibleFieldIds,
      scopeKey,
      actions,
    } = await collectNewFields(
      page,
      pageEntry,
      registry,
      runId,
    ...
    if (actions?.length) {
      if (pageEntryExisting) {
        if (
          !pageEntryExisting.actions ||
          pageEntryExisting.actions.length === 0
        ) {
    ...
    const baselineForAfterScope =
      visibleBaselineByScope.get(scopeKey) ?? new Set<string>();
    const sameScope = lastScope.key === scopeKey;
    const visibilityDiff = sameScope
      ? diffFieldSets(lastVisibleFieldIds, visibleAfter)
      : diffFieldSets(baselineForAfterScope, visibleAfter);
    ...
    if (visibleBaselineByScope.has(scopeKey)) {
      const merged = new Set<string>([
        ...visibleBaselineByScope.get(scopeKey)!,
        ...visibleAfter,
      ]);
    ...
    const fieldStateSnapshot = await captureFieldStateSnapshot(
      page,
      snapshotDescriptors,
      afterScope.root,
    );
    ...
    clickLogEntries.push({
      ...
      newFieldIds,
      newlyVisibleFieldIds,
      newlyDiscoveredFieldIds:
        clickKind === "system_alert" ? [] : newlyDiscoveredFieldIds,
