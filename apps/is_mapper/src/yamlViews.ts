...
function formatField(field: FieldEntry): Record<string, unknown> {
  ...
  return {
    ...
    valueQuality: field.valueQuality,
    opensModal: field.opensModal,
...
export function buildNavigationView(
  map: UiMap,
...
export function buildYamlViews(map: UiMap): {
...
export function validateMapForYaml(
  ...
  const missingLabelFields = map.fields.filter(
    (field) => !(field.label ?? "").trim(),
  );
  if (missingLabelFields.length > 0) {
    const sample = missingLabelFields
      .slice(0, 5)
      .map((field) => field.id)
      .join(", ");
    throw new Error(
  ...
  const longBreadcrumbs = breadcrumbs.filter(
    (breadcrumb) => breadcrumb.length > 80,
  );
