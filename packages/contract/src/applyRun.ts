...
export const ApplyRunStatusSchema = z.enum(["started", "completed", "partial", "failed"]);
...
export const ApplyRunArtifactSchema = z.object({
  schemaVersion: z.literal(APPLY_RUN_SCHEMA_VERSION).default(APPLY_RUN_SCHEMA_VERSION),
  accountNumber: z.string().trim().min(1),
  variation: z.string().trim().min(1),
  deviceIp: z.string().trim().min(1),
  mapPath: z.string().trim().min(1),
