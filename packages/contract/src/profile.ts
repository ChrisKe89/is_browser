...
export const ProfileValuesMapSchema = z.record(
  z.string().trim().min(1),
  z.string(),
);
...
export const ProfileArtifactSchema = ProfileIdentitySchema.extend({
  schemaVersion: z
    .literal(PROFILE_SCHEMA_VERSION)
...
export type ApplyProfileSetting = z.infer<
  typeof ApplyProfileSettingsSchema
>[number];
...
export function profileMapToApplySettings(
