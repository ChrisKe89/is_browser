...
export const AUTH_STATE_PATH =
  process.env.AUTH_STATE_PATH ?? "state/auth-state.json";
export const USE_AUTH_STATE =
  (process.env.USE_AUTH_STATE ?? "false").toLowerCase() !== "false";
export const REMOTE_PANEL_URL = process.env.REMOTE_PANEL_URL ?? "";
export const REMOTE_PANEL_PROFILE =
  process.env.REMOTE_PANEL_PROFILE ?? "default";
export const DISCOVERY_SUBNET = process.env.DISCOVERY_SUBNET ?? "192.168.0";
export const DISCOVERY_RANGE_START = Number(
  process.env.DISCOVERY_RANGE_START ?? 1,
);
export const DISCOVERY_RANGE_END = Number(
  process.env.DISCOVERY_RANGE_END ?? 254,
);
export const DISCOVERY_TIMEOUT_MS = Number(
  process.env.DISCOVERY_TIMEOUT_MS ?? 250,
);
...
export const FORM_PUBLIC_URL =
  process.env.FORM_PUBLIC_URL ?? `http://localhost:${FORM_PORT}`;
export const DEVICE_LOG_MODE = (process.env.DEVICE_LOG_MODE ?? "all-time") as
  | "all-time"
  | "daily";
export const CUSTOMER_MAP_CSV =
  process.env.CUSTOMER_MAP_CSV ??
  "../../tools/samples/devices/customer-map.csv";
export const PROFILE_DB_PATH =
  process.env.PROFILE_DB_PATH ?? "state/profile-runner.sqlite";
...
export const CRAWL_FLOWS_PATH =
  process.env.CRAWL_FLOWS_PATH ?? "config/crawler-flows.json";
