...
const rawCapturePath = process.env.MANUAL_CAPTURE_PATH ?? "state/manual-captures.json";
const manualMapPath = process.env.MANUAL_MAP_PATH ?? "state/printer-ui-map.manual.json";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    output.write("\nManual variation capture is ready.\n");
    output.write("Open any page/modal in the browser, then capture from terminal.\n");
    output.write("Commands: c = capture, d = done and write files, q = quit without writing\n");

    let sequence = captures.length + 1;
    while (true) {
      const cmd = (await rl.question("\nCommand [c/d/q]: ")).trim().toLowerCase();
      if (cmd === "q") {
        output.write("Quit without writing.\n");
        return;
      }
      if (cmd === "d") break;
      if (cmd && cmd !== "c") {
      ...
      await closeDialogs(page);
      await installClickLogger(page);
      const note = (await rl.question("Note (optional): ")).trim() || undefined;
      const { modalTitles, controls } = await collectControls(page);
      const clicks = await pullClicks(page);
      ...
      output.write(
        `Captured ${capture.id}: ${controls.length} controls, ${clicks.length} clicks, URL ${capture.url}\n`
      );
    }
    ...
    output.write(
      `\nWrote ${captures.length} captures to ${rawCapturePath}\n` +
        `Wrote ${map.pages.length} pages / ${map.fields.length} fields to ${manualMapPath}\n`
    );
  } finally {
    rl.close();
    await context.close();
